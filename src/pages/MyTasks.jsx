...
const MyTasks = () => {
  ...
  const dropdownTypes = [
    { id: 'all', label: 'Усі типи', icon: <SlidersHorizontal size={14} /> },
    { id: 'user', label: 'Канали', icon: <UserPlus size={14} className="text-[#38BDF8]" /> },
    { id: 'group', label: 'Групи', icon: <Users size={14} className="text-[#A78BFA]" /> },
    { id: 'reaction', label: 'Реакції', icon: <Flame size={14} className="text-[#F472B6]" /> },
    { id: 'bot', label: 'Боти', icon: <Bot size={14} className="text-[#34D399]" /> },
    { id: 'view', label: 'Перегляди', icon: <Eye size={14} className="text-[#60A5FA]" /> },
    { id: 'boost', label: 'Бусти', icon: <Zap size={14} className="text-[#FBBF24] fill-[#FBBF24]/10" /> }
  ];

  const getIcon = (type, color) => {
    switch (type) {
      case 'bot':
        return <Bot size={18} style={{ color }} />;
      case 'view':
        return <Eye size={18} style={{ color }} />;
      case 'boost':
        return <Zap size={18} className="fill-current" style={{ color }} />;
      case 'reaction':
        return <Flame size={18} style={{ color }} />;
      default:
        return <User size={18} style={{ color }} />;
    }
  };
    ...
    <Layout showNavbar={false}>
      <div className="flex flex-col min-h-screen text-white pb-6">
        ...
        <div className="flex items-center justify-between border-b border-b-[#232637] p-[22px_15px] mb-6">
          ...
          <h1 className="text-base font-semibold text-white tracking-wider flex-1 text-center mr-5">My tasks</h1>
        </div>
        ...
        <div className="px-5 flex-1 flex flex-col gap-4 relative">
          ...
          <div className="flex items-center justify-between gap-1 py-1 relative">
            {/* Scrollable Tabs Wrapper */}
              ...
              >
                <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${activeTab === 'active' ? 'bg-[#0B0E21]/10' : 'bg-white/5'}`}>
                  <Play size={10} className={activeTab === 'active' ? 'fill-[#0B0E21] text-[#0B0E21]' : 'fill-[#8A8D9F] text-[#8A8D9F]'} />
                </div>
                <span>(6)</span>
              </button>
              ...
              >
                <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${activeTab === 'ready' ? 'bg-[#0B0E21]/10' : 'bg-white/5'}`}>
                  <Check size={10} className={activeTab === 'ready' ? 'text-[#0B0E21]' : 'text-[#8A8D9F]'} strokeWidth={3} />
                </div>
                <span>(4)</span>
              </button>
              ...
              >
                <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${activeTab === 'paused' ? 'bg-[#0B0E21]/10' : 'bg-white/5'}`}>
                  <div className="flex items-center gap-0.5">
                    <div className={`w-0.5 h-2 rounded-full ${activeTab === 'paused' ? 'bg-[#0B0E21]' : 'bg-[#8A8D9F]'}`}></div>
                    <div className={`w-0.5 h-2 rounded-full ${activeTab === 'paused' ? 'bg-[#0B0E21]' : 'bg-[#8A8D9F]'}`}></div>
                  </div>
                </div>
                <span>(3)</span>
            ...
            {showDropdown && (
              <div 
                className="absolute right-0 top-full mt-2 w-[180px] bg-[#161829] border border-[#2A2D40] rounded-[16px] p-1.5 flex flex-col gap-0.5 z-50 shadow-[0_10px_30px_rgba(0,0,0,0.5)] backdrop-blur-md"
              >
            ...
            {filteredTasks.map((task) => {
              ...
              return (
                <Link 
                  ...
                  state={task}
                  key={task.id}
                  className="gradient-border-card rounded-[16px] p-3 flex items-center gap-3.5 hover:opacity-95 transition-opacity cursor-pointer block"
                ...
                >
                  ...
                  <div 
                    ...
                    style={{ 
                      ...
                      boxShadow: `0 0 12px ${task.iconColor}15`
                    }}
                  >
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    ...
                    <div className="flex items-center justify-between w-full">
                      <h4 className="text-[14px] font-semibold text-white truncate max-w-[120px]">
                        {task.name}
                      </h4>
                      <div className="flex items-center">
                        <span className="text-[12px] text-[#5A5D72] font-medium mr-4">
