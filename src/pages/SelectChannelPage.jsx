  ...
  return (
    <Layout showNavbar={false}>
      <div className='flex flex-col font-sans text-white relative min-h-screen'>
        <div className='p-[10px_15px_22px_15px] border-b border-b-white/10'>
          ...
          <div className='relative w-full mb-5'>
            ...
            <input
              ...
              className='w-full h-[48px] rounded-full bg-[#0B0E21]/40 border border-white/5 pl-11 pr-4 text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#87ADF0] transition-colors'
            />
          </div>
        </div>

        {/* Channels List */}
        <div className='flex flex-col gap-3 mb-8 pt-4 px-[22px]'>
          {filteredChannels.map((channel) => (
            <div
              key={channel.id}
              onClick={() => navigate('/duration-setup', { state: { taskType: 'channel_sub', channel: channel.username } })}
              className='gradient-border-card rounded-xl p-3 flex items-center justify-between cursor-pointer hover:bg-white/[0.02] active:scale-[0.99] transition-all'
        ...
        <div className='mt-auto px-[18px_32px]'>
          <button
            onClick={() => navigate('/duration-setup', { state: { taskType: 'channel_sub', channel: '@crypto_signals_ua' } })}
