...
import { ChevronLeft, ChevronRight, MoreVertical, Check, AlertTriangle, EyeOff, Flag, Clipboard, Image as ImageIcon, Drama, Ban, Smartphone, Unlink, Star, PenTool } from 'lucide-react';
...
const CATEGORIES = [
  {
    id: 'webapp',
    name: 'Боти з Web App',
    description: 'Боти з інтеграцією веб-додатків',
    count: 120,
    icon: '/assets/images/vector16.png',
  },
  {
    id: 'standard',
    name: 'Стандартні боти',
    description: 'Звичайні Telegram боти без додаткових веб-інтерфейсів',
    count: 75,
    icon: '/assets/images/vector28.png',
  },
  {
    id: 'conditions',
    name: 'З додатковими умовами',
    description: 'Потрібно виконати дії: капча, мова, підписка на канали',
    count: 229,
    icon: '/assets/images/vector48.png',
  }
];

const BOTS_BY_CATEGORY = {
  webapp: [
    { id: 'bot_wa_1', name: '@crypto_trade_bot', reward: 1750 },
    { id: 'bot_wa_2', name: '@game_earn_bot', reward: 1000 },
    { id: 'bot_wa_3', name: '@wallet_app_bot', reward: 750 },
  ],
  standard: [
    { id: 'bot_st_1', name: '@verify_member_bot', reward: 500 },
    { id: 'bot_st_2', name: '@official_community_bot', reward: 450 },
  ],
  conditions: [
    { id: 'bot_co_1', name: '@adv_channel_bot', reward: 2200 },
    { id: 'bot_co_2', name: '@claim_drop_bot', reward: 1900 },
  ]
};
...
const ExecuteBot = () => {
  ...
  const [selectedCatId, setSelectedCatId] = useState('webapp');
  ...
  const currentBotsList = BOTS_BY_CATEGORY[selectedCatId] || [];
  ...
  const handleBotAction = () => {
    if (selectedBot) {
      alert(`Перехід до бота ${selectedBot.name}...`);
    }
  };

  const handleSecondaryAction = (actionType) => {
    alert(`Дія "${actionType}" активована для ${selectedBot?.name}`);
  };
        ...
        <div className="flex flex-col items-center pt-8 pb-5 px-5 relative border-b border-white/10 shrink-0">
          ...
          {viewState === 'detail' && menuOpen && (
            ...
            >
              <button
                onClick={() => {
                  alert(`Завдання ${selectedBot?.name} пропущено`);
                  handleBack();
                }}
          ...
          {viewState === 'categories' && (
            <>
              <img src="/assets/images/roboLine.png" alt="roboLine" className='mb-3' />
          ...
          {viewState === 'detail' && (
            <>
              <img src="/assets/images/roboLine.png" alt="bot avatar" className='mb-3' />
              ...
              {currentBotsList.map((bot) => (
                ...
                >
                  <div className="flex items-center gap-3.5">
                    <img src="/assets/images/vector28.png" alt="bot list avatar" className="w-11" />
                    <div>
                      <h3 className="font-semibold text-[15px] text-white leading-tight">
                        {bot.name}
                      </h3>
            ...
            <div className="px-1">
              ...
              >
                ...
                <p className="text-xs text-white/40 leading-snug font-normal">
                  Перейдіть в бота, запустіть його та зробіть скриншот для підтвердження виконання.
                </p>

                {/* Main Action Button */}
                <button
                  onClick={handleBotAction}
                  className="w-full h-11 rounded-[30px] font-semibold text-sm flex items-center justify-center gap-2 text-[#0B0E21] bg-[#82A7F9] hover:brightness-110 active:scale-[0.98] transition-all cursor-pointer shadow-[0_4px_16px_rgba(130,167,249,0.2)]"
                >
                  <img src="/assets/images/svg/SmileSquare-black.svg" alt="SmileSquare-black" className="w-5" />
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  { id: 'inappropriate', label: 'Неприйнятний контент', icon: '/assets/images/svg/Forbidden.svg' },
                  { id: 'no_screenshot', label: 'Заборонено робити скріншот', icon: '/assets/images/svg/smartphone.svg' },
                  { id: 'broken_link', label: 'Непрацююче посилання', icon: '/assets/images/svg/LinkMinimalistic.svg' },
                  { id: 'paid_reactions', label: 'Платні реакції', icon: '/assets/images/svg/star2.svg' },
                  { id: 'other', label: 'Інша причина', icon: '/assets/images/svg/DocumentAdd2.svg' },
                ].map((reason) => {
              ...
              {selectedReason === 'other' && (
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-2xl bg-[#090B19]/60 border border-white/8 px-4 py-2.5 text-[13px] text-white placeholder-white/40 focus:outline-none focus:border-[#FF4973]/30 resize-none font-medium mt-1 leading-normal"
                />
              )}
